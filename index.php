...
<?php
elseif (preg_match('/^\/(' . implode('|', array_keys($channel)) . ')\/(\s*|res\/(\d+)\.html)?$/', $_SERVER['REQUEST_URI'], $match)):
	$forum = $match[1];
	$id = isset($match[3]) ? $match[3] : NULL; ?>
<?php
	$dbh = newSQLite3(dirname($webroot) . '/' . $forum . '.db');
	$dbh->busyTimeout(5000);
	$dbh->query('PRAGMA synchronous=NORMAL;');
	$dbh->query('PRAGMA journal_mode=WAL;');
	$dbh->query(' CREATE TABLE IF NOT EXISTS "forum" ( "post_id" INTEGER NOT NULL, "forum_id" VARCHAR(255) NOT NULL, "post_date" DATETIME DEFAULT NULL, "post_date_created" DATETIME DEFAULT CURRENT_TIMESTAMP, "post_date_modified" DATETIME DEFAULT NULL, "post_date_bumped" DATETIME DEFAULT NULL, "post_subject" VARCHAR(255) DEFAULT NULL, "post_body" TEXT DEFAULT NULL, "post_text" TEXT DEFAULT NULL, "post_password" TEXT DEFAULT NULL, "post_attach" VARCHAR(255) DEFAULT NULL, "post_author" INTEGER DEFAULT NULL, "post_author_name" VARCHAR(255) DEFAULT NULL, "post_author_email" VARCHAR(255) DEFAULT NULL, "post_author_jabberid" VARCHAR(255) DEFAULT NULL, "post_author_www" VARCHAR(255) DEFAULT NULL, "post_author_ip" VARCHAR(45) DEFAULT NULL, "post_agent" VARCHAR(255) DEFAULT NULL, "thread" VARCHAR(255) DEFAULT NULL, "multithread" VARCHAR(255) DEFAULT NULL ) '); ?>
<?php
	if (isset($id))
		{
		$try = $dbh->prepare('SELECT * FROM forum WHERE post_id = :post_id');
		$try->bindValue(':post_id', $id);
		if ($row = $try->execute()->fetchArray()) extract($row);
		unset($row);
		}
	...
	if ($_SERVER['REQUEST_METHOD'] === 'POST')
		...
		if (isset($_POST['delete']))
			{
			if (empty($_POST['password']) || is_array($_POST['delete']) === FALSE)
				{
				die('invalid request');
				}
		...
		$csrf = md5(serialize(array(
			'IP' => $_SERVER['REMOTE_ADDR'],
			session_name() => session_id() ,
			'salt' => $webroot
		)));
		if (@$_POST['CSRF'] !== $csrf) die('YOU SHALL NOT PASS');
		$req = $dbh->query('SELECT COUNT(*) FROM forum WHERE post_author_ip = "' . $_SERVER['REMOTE_ADDR'] . '" AND post_date_created > datetime("now", "-5 minutes")')->fetchArray() ['COUNT(*)'];
		if ($req >= 5) die('STOP FLOOD');
		...
		if (empty($_POST['nofile']) && ($files = count($_FILES['file']['tmp_name'])) > 0)
			{
			if (isset($_FILES['file']['error']) === FALSE || is_array($_FILES['file']['error']) === FALSE)
				{
				die('invalid request');
				}
		...
		function ($match)
			...
			if (isset($res))
				{
				return '<a href="/' . $forum . '/res/' . explode('.', $res['thread']) [0] . '.html#' . $match[1] . '">' . $match[0] . '</a>';
				}
		...
		foreach($ref['multithread'] as $ref_id => $ref_n)
			...
			foreach($ref_n as $n => $ref_multithread)
				...
				if (@$last_multithread === $multithread) continue;
				$last_multithread = $multithread;
	...
	if ($_SERVER['REQUEST_METHOD'] === 'GET'): ?>
		...
		echo $channel[($forum) ]; ?>
                        ...
                        <body>
                            <div class="wrap">
                                [
                                <?php
		foreach($channel as $k => $v): ?> <a href="/<?php
			echo $k; ?>/">/<?php
			echo $k; ?>/</a>
                                <?php
		endforeach; ?> ]
		...
		endforeach; ?>
                                                    ...
                                                    <tr>
                                                        ...
                                                        <td>
                                                            ...
                                                            <input type="checkbox" name="nofile"> No file]</td>
                                                    </tr>
                                                    <tr>
                                                        <th>Password</th>
                                                        <td>
                                ...
                                <div class="centered_wrap">
                                    <div class="centered_text">
                                        <ul>
                                            ...
                                            <li>Maximum file size allowed is 9000KB.</li>
                                            <li>Images greater than 320x240 pixels will be thumbnailed.</li>
                                            <li>This is a work-safe forum. Adult images are forbidden.</li>
                                        </ul>
                                    </div>
                                </div>
                                <div class="clear"></div>
		...
		if ($id === NULL): ?>
                                    <?php
			$try_posts = $dbh->query('SELECT * FROM forum WHERE multithread = post_id ORDER BY post_date_bumped DESC, post_id DESC'); ?>
                                    <?php
			while ($row = $try_posts->fetchArray()):
				extract($row);
				unset($row); ?>
                                    <div class="thread">
                                        <a name="<?php
				...
				echo (empty($post_author) && empty($post_author_name)) ? 'anonymous' : $post_author_name; ?></b> <u><?php
				echo $post_author ? '!' . $post_author : ''; ?></u>
                                        <?php
				echo $post_date_created; ?> #
                                        <?php
				echo $post_id; ?> [<a href="/<?php
				echo $forum; ?>/res/<?php
				echo $post_id; ?>.html">Reply</a>]
                                        <?php
				if (isset($post_attach)): ?>
                                        <?php
					foreach(explode(' ', $post_attach) as $i): ?><a href="/<?php
						echo $forum; ?>/src/<?php
						echo $i; ?>"><img src="/<?php
						echo $forum; ?>/thumb/<?php
						echo $i; ?>"></a>
                                        <?php
					endforeach; ?>
                                        <?php
				...
				echo $post_body; ?>
                                        <?php
				$try_thread = $dbh->query('SELECT * FROM forum WHERE multithread = "' . explode('.', $multithread) [0] . '" OR multithread LIKE "' . explode('.', $multithread) [0] . '.%" GROUP BY multithread ORDER BY multithread'); ?>
                                        <?php
				while ($row = $try_thread->fetchArray()):
					extract($row);
					unset($row); ?>
                                        <?php
					echo str_repeat('<div class="re">', substr_count($multithread, '.')); ?>Re: <a href="/<?php
					echo $forum; ?>/res/<?php
					echo $post_id; ?>.html"><?php
					echo str_replace(array(
						'&',
						'<',
						'>',
						...
						'&gt;',
						' '
					) , mb_substr($post_text, 0, 80, 'utf-8')) . (mb_strlen($post_text, 'utf-8') > 80 ? ' ...' : ''); ?></a>
                                        <?php
					echo str_repeat('</div>', substr_count($multithread, '.')); ?>
                                        <?php
				endwhile; ?>
                                    </div>
                                    <hr>
		...
		elseif (isset($post_id)): ?>
				...
				if ((@++$row_count === 1) && ($thread == $post_id)): ?>
					...
					if (isset($post_attach)): ?>
						...
						endforeach; ?>
                                        ...
                                        <br />
                                        <?php
					if ($row = $dbh->query('SELECT * FROM forum WHERE post_id = ' . array_reverse(explode('.', $multithread)) [1] . '')->fetchArray()): ?>Re: <a href="/<?php
						echo $forum; ?>/res/<?php
						echo $row['post_id']; ?>.html"><?php
						echo str_replace(array(
							'&',
							'<',
							'>',
							...
							'&gt;',
							' '
						) , mb_substr($row['post_text'], 0, 80, 'utf-8')) . (mb_strlen($row['post_text'], 'utf-8') > 80 ? ' ...' : ''); ?></a>
                                        <?php
					endif; ?>
                                        <?php
					echo $post_body; ?>
			...
			endwhile; ?>
                                    ...
                                    <hr>
                                    <?php
		else: ?> Thread ID not found.
                                    <hr>
                                    <?php
		endif; ?> Password
                                    <input type="password" name="password"> [
                                    <input type="checkbox" name="fileonly"> File only]
